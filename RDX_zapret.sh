  ...
  if download_release "$version" "$archive"; then
    ...
    if tar -xzf "$archive" -C "$temp_dir" 2>/dev/null; then
      ...
      if [ "$TEST_MODE" = "false" ]; then
        ...
        # === rdx-zapret PATCh для install_easy.sh ===
        print_info "Применение патча rdx-zapret к install_easy.sh..."
        ...
        # === rdx-zapret PATCh для uninstall_easy.sh ===
        print_info "Применение патча rdx-zapret к uninstall_easy.sh..."
        if [ -f "$actual_path/uninstall_easy.sh" ]; then
          ...
          print_success "Патч rdx-zapret применён к uninstall_easy.sh (remove_extra_pkgs_openwrt закомментирована)"
        fi
      fi
        ...
        if [ "$TEST_MODE" = "true" ]; then
          ...
          if download_release "$latest_version" "$archive"; then
            ...
            if tar -xzf "$archive" -C "$temp_dir" 2>/dev/null; then
              local found=false
              for dir in \
                "$temp_dir/zapret/binaries/linux-arm" \
  ...
  while true; do
    ...
    if [ -d "$INSTALL_PATH" ] && [ -n "$(ls -A "$INSTALL_PATH" 2>/dev/null)" ]; then
      if is_zapret_running; then
        ...
        echo -e "${RED}1. Остановить zapret ${NC}(вкл/выкл)"
      else
      ...
      echo -e "${RED}7. Полностью удалить zapret${NC}"
      echo ""
